...
- `AGENTS.md` (root) — committed, self-sufficient repo guidance; read it
  ...
  this repo's policy, so keep nested files purely additive: they must never
...
## Bug fixing (Spec Kit "bug" extension)

For broken existing behavior (not new features) use the opt-in bug
extension. `agentic_setup.py install --specify` installs it
automatically; otherwise install it once per machine:

```bash
specify extension add bug
```

Run the three stages in your agent, one slug per bug; each stage writes a
durable record in `.specify/bugs/<slug>/` (committed — see the table
above and `.gitignore`):

1. `/speckit-bug-assess "<symptom>" slug=<slug>` → `assessment.md` —
   diagnosis and remediation decision; **no source changes** in this
   stage. Review the diagnosis before asking for a fix.
2. `/speckit-bug-fix slug=<slug>` → `fix.md` — the only stage that edits
   source; work outside the assessed scope must be recorded as a
   deviation, never silently expanded.
...
Guide: <https://github.github.com/spec-kit/guides/bugfix.html>

## Keep it clean

- Commit only durable decision records and the root guidance; never commit
  per-machine or generated artifacts (see the table above).
